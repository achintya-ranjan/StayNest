...
const PhotosUploader = ({addedPhoto, onChange}) => {
    ...
    function selectAsMainPhoto(ev,filename) {
        ...
        onChange([filename,...withoutSelected]);
    }
     */ 
  return (
    <>
        ...
        <div className="mt-2 grid gap-2  grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
            {addedPhoto.length>0 && addedPhoto.map(link => (
                <div className="h-32 flex relative" key={link}>
                    <img className="rounded-2xl w-full object-cover" src={'http://localhost:4000/uploads/'} alt=""/>
                    {/*Adding a trash icon in each picture*/}
                    <button onClick={(event)=>removePhoto(event,link)} className="cursor-pointer absolute bottom-1 right-1 text-white bg-black bg-opacity-50 rounded-xl py-2 px-3">
            ...
            <label className="h-32 cursor-pointer flex items-center justify-center gap-1 border bg-transparent rounded-2xl p-2 text-2xl text-gray-600"> 
                <input type="file" multiple className="hidden" onClick={uploadPhoto}/>
